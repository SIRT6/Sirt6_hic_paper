...
save(res_p, file = "anno_atac_dif_promoter.RData")
...
save(res_p, file = "anno_atac_promoter.RData")
...
save(go_plots_p_BP, file = 'go_plots_p_BP.RData')

load('go_plots_p_BP.RData')
go_plots_p_BP
...
abs_plots <- function(ego_result, categories, anno_result, name_of_dif) {
  ...
  for (i in 1:length(categories)) {
    ...
    category_df <- category_df[category_df$log2FoldChange*category_df$geneLFC > 0,]
...
save(KO_WT_categories_abs_p, file = 'KO_WT_categories_abs_p.RData')  

ggarrange(KO_WT_categories_abs_p$plots[[1]], KO_WT_categories_abs_p$plots[[2]], KO_WT_categories_abs_p$plots[[3]],
          ncol = 3, nrow = 1)

unique_KO_WT_cat <- rbind(KO_WT_categories_abs_p$genes[[1]], 
                          KO_WT_categories_abs_p$genes[[2]])

for (i in 3:length(KO_WT_categories_abs_p$genes)) {
...
unique_KO_WT_cat_names <- getBM(attributes = c('ensembl_gene_id', "external_gene_name"),
                                ...
                                mart = mart)
unique_KO_WT_cat_names_2 <- getBM(attributes = c('ensembl_gene_id', "external_gene_name", 'description'),
                                  filters = "ensembl_gene_id",
                                  values = unique_KO_WT_cat$ensembl_gene_id,
                                  mart = mart)
unique_KO_WT_cat_names[nrow(unique_KO_WT_cat_names)+1,] <- c('ENSMUSG00000030102', 'Itpr1')
unique_KO_WT_cat_names <- unique_KO_WT_cat_names[order(unique_KO_WT_cat_names$ensembl_gene_id),]
# unique_KO_WT_cat_names[nrow(unique_KO_WT_cat_names)+1,] <- unique_KO_WT_cat_names[nrow(unique_KO_WT_cat_names),]    #in all 4 quarters
...
ggplot(unique_KO_WT_cat, aes(log2FoldChange, geneLFC, label = gene_name)) +
  ...
  geom_text_repel(aes(label = gene_name),
                  ...
                  segment.size  = 0.1,
                  size = 2.5)
ggsave(('LFC_KO_WT_neuro_unique_3.png'), 
       width = 6, height = 6, dpi = 500)



write.csv(unique_KO_WT_cat, file = 'unique_KO_WT_neuro.csv', sep = '\t')
